                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Poids Initial (kg)</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <c:forEach items="${poissons}" var="p">
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${p.poidsInitial} kg</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <c:choose>
                                            <c:when test="${p.currentStatut.nom == 'Vivant'}">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                                    ${p.currentStatut.nom}
                                                </span>
                                            </c:when>
                                            <c:when test="${p.currentStatut.nom == 'Vendu'}">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                                    ${p.currentStatut.nom}
                                                </span>
                                            </c:when>
                                            <c:when test="${p.currentStatut.nom == 'Mort'}">
